...
def main(config_filename: str):
    ...
    queue_akces = manager_akces.Queue(4)
    gel_akces = None
    error_generator_akces = None

    process = Process(
                target=load_data.data_generator, 
                args=(queue_akces, DATA_PATHS, NUM_PARALLEL, gel_akces, tokenizer, MAX_LENGTH, 
                      ERRORS_FROM_FILE, REVERTED_PIPELINE, error_generator_akces, LANG, 
                      COUNT_OUTPUT, ))
    process.start()
    ...
    dataset_akces = dataset_akces.map(dataset_utils.split_features_and_labels, 
                                      num_parallel_calls=tf.data.experimental.AUTOTUNE)
    dataset_akces = dataset_akces.shuffle(SHUFFLE_BUFFER)
    dataset_akces = dataset_akces.bucket_by_sequence_length(
            element_length_func=lambda x, y: tf.shape(x['input_ids'])[0], # zde asi chyba
            bucket_boundaries=BUCKET_BOUNDARIES,
            bucket_batch_sizes=bucket_batch_sizes
    )
    dataset_akces = dataset_akces.map(
        lambda x, y: dataset_utils.change_value(x, y, 0, LABEL_PAD_VALUE, MODEL_TYPE))
    dataset_akces = dataset_akces.prefetch(tf.data.experimental.AUTOTUNE)
    ###

    for i in dataset_akces:
        print(i)
